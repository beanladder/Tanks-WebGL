...
public class NetworkProjectileLauncher : MonoBehaviourPunCallbacks
{
    ...
    void Update()
    {
        if (view.IsMine)
        {
            if (Input.GetMouseButtonDown(0) && canFirePrimary)
            {
                ...
                // Call FireProjectile locally
                FireProjectile(shooterID, firePoint.position, firePoint.rotation, velocity);
                
                // Synchronize the projectile spawn across the network
...

    {
        ...
        Destroy(trailEffect, trailDuration);
        GameObject smoke = Instantiate(BarrelSmokePrefab, position, rotation);
        GameObject flash = Instantiate(BarrelFlashPrefab, position, rotation);
        Destroy(flash, 2f);
        Destroy(smoke, 2f);
    }
